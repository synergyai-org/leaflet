...
import DesignTab from "./admin/tabs/DesignTab";
import ProgramsTab from "./admin/tabs/ProgramsTab";
import AdTab from "./admin/tabs/AdTab";
import LeafletList from "./admin/LeafletList";
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  // Added originalId to track potential ID changes during editing
  ...
  // Fixed: Added handleClone to satisfy LeafletListProps requirement
  const handleClone = async (metadata: LeafletMetadata) => {
    ...
    try {
      ...
      const result = await gasService.saveLeaflet(
        newId,
        newTitle,
        targetConfig,
        true,
      ...
      if (result.success) {
        setToast({
          message: "리플렛이 성공적으로 복제되었습니다.",
          type: "success",
        });
        await loadList();
      } else {
        setToast({
          message: result.message || "복제에 실패했습니다.",
          type: "error",
        });
      }
    } catch (e) {
      setToast({
        message: "복제 과정에서 오류가 발생했습니다.",
        type: "error",
      });
    } finally {
      setIsSaving(false);
  ...
  const handleSave = async () => {
    ...
    if (!idRegex.test(currentId)) {
      setToast({
        message: "ID에는 영문, 숫자, 하이픈(-), 언더바(_)만 가능합니다.",
        type: "error",
      });
      return;
    }
    ...
    if (result.success) {
      ...
      // If the currently viewed leaflet was updated, refresh the view
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      if (success) {
        ...
        setToast({
          message: "삭제 요청을 처리하지 못했습니다. ID를 확인해주세요.",
          type: "error",
        });
      }
    } catch (e) {
      setToast({
        message: "삭제 중 서버 통신 오류가 발생했습니다.",
        type: "error",
      });
    } finally {
      setIsSaving(false);
        ...
        <div className="max-w-5xl mx-auto w-full">
          <div className="flex justify-between items-center mb-10">
            <div>
              ...
              <p className="text-xs text-slate-400 tracking-[3px] uppercase mt-1">
            ...
            <div className="flex gap-4">
              <button
                ...
                disabled={isSaving}
                className="bg-[#B0925A] text-white px-6 py-3 rounded-sm font-bold text-xs tracking-[2px] hover:bg-[#E1B16A] transition-all uppercase disabled:opacity-50"
              >
                New Leaflet
              </button>
              <button
                onClick={onClose}
                className="text-slate-400 hover:text-slate-800 uppercase text-[10px] tracking-[2px] px-4"
              >
                Close
              </button>
            </div>
          ...
          <LeafletList
            ...
            onClone={handleClone} // Satisfy type requirement
            isSaving={isSaving}
          />
        </div>
    ...
    <div className="fixed inset-0 z-[300] bg-white flex flex-col md:flex-row text-slate-800 animate-in fade-in zoom-in duration-300">
      <div className="w-full md:w-80 bg-slate-900 text-white p-8 flex flex-col shrink-0">
        <button
          ...
          className="mb-8 text-[10px] text-white/30 hover:text-white transition-all uppercase tracking-[2px] flex items-center gap-2"
        >
        ...
        <div className="mb-10 space-y-4">
          ...
          <div>
            <label className="text-[9px] text-[#B0925A] uppercase tracking-widest block mb-1">
              URL Slug (Leaflet ID)
            </label>
        ...
        <nav className="flex flex-col gap-2 flex-1 overflow-y-auto no-scrollbar">
          {(["general", "design", "programs", "ad"] as const).map((tab) => (
            <button
              ...
              className={`text-left p-4 rounded transition-all uppercase tracking-widest text-[10px] font-bold border-l-2 ${activeTab === tab ? "bg-[#B0925A]/10 border-[#B0925A] text-[#B0925A]" : "border-transparent hover:bg-white/5 opacity-50"}`}
            >
              {tab === "general"
                ? "일반 설정"
                : tab === "design"
        ...
        <div className="mt-8 pt-8 border-t border-white/10 flex flex-col gap-3">
          <button
            onClick={handleSave}
            disabled={isSaving}
            className="w-full bg-[#B0925A] hover:bg-[#E1B16A] text-black font-bold py-4 rounded-sm transition-all text-xs tracking-[3px] shadow-lg disabled:opacity-50"
          >
          ...
          <button
            ...
            className="w-full bg-red-900/20 text-red-400 hover:bg-red-900/40 py-3 rounded-sm text-[9px] tracking-[2px] uppercase"
          >
            초기화
          </button>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-8 md:p-16 bg-slate-50 no-scrollbar">
        <div className="max-w-3xl mx-auto">
          {activeTab === "general" && (
            <GeneralTab
              config={localConfig}
              onUpdate={updateStyledField}
              onSetRoot={setLocalConfig}
            />
          )}
          {activeTab === "design" && (
            <DesignTab config={localConfig} onSetRoot={setLocalConfig} />
          )}
          {activeTab === "programs" && (
            <ProgramsTab config={localConfig} onSetRoot={setLocalConfig} />
          )}
          {activeTab === "ad" && (
            <AdTab config={localConfig} onSetRoot={setLocalConfig} />
          )}
        </div>
      </div>
      {toast && <Toast message={toast.message} type={toast.type} />}
