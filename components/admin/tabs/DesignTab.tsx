...
const DesignTab: React.FC<DesignTabProps> = ({ config, onSetRoot }) => {
  const updateTheme = (field: string, val: any) => onSetRoot({ ...config, globalTheme: { ...config.globalTheme, [field]: val } });
  const updateLogos = (field: string, val: any) => onSetRoot({ ...config, logos: { ...config.logos, [field]: val } });
  const updateBg = (field: string, val: any) => onSetRoot({ ...config, backgrounds: { ...config.backgrounds, [field]: val } });

  const brightness = config.backgrounds.heroBgBrightness ?? 0.5;
  const contrast = config.backgrounds.heroBgContrast ?? 1.1;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 border-b pb-6">
        <div>
          ...
          <input type="color" className="w-full h-10 rounded cursor-pointer" value={config.globalTheme.accentColor} onChange={e => updateTheme('accentColor', e.target.value)} />
        ...
        <div>
          ...
          <input type="color" className="w-full h-10 rounded cursor-pointer" value={config.globalTheme.bgColor} onChange={e => updateTheme('bgColor', e.target.value)} />
        </div>
      </div>
      ...
      <div className="space-y-4 border-b pb-6">
        ...
        <div className="flex items-center gap-3 mb-4 p-3 bg-yellow-50 rounded border border-yellow-100">
          <input type="checkbox" id="applyFilter" checked={config.logos.applyFilter} onChange={e => updateLogos('applyFilter', e.target.checked)} />
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <div className="p-3 border rounded bg-white">
            ...
            <input className="w-full border rounded p-2 text-xs font-mono mb-2" value={config.logos.leftLogo} onChange={e => updateLogos('leftLogo', e.target.value)} />
            <label className="block text-[10px] font-bold mb-1 uppercase text-indigo-600">클릭 이동 링크</label>
            <input className="w-full border border-indigo-200 rounded p-2 text-xs font-mono" value={config.logos.leftLink} onChange={e => updateLogos('leftLink', e.target.value)} />
            <SpecGuide spec="600px+ Width, Transparent PNG" />
            <ImagePreview url={config.logos.leftLogo} label="Left Logo" />
          </div>
          <div className="p-3 border rounded bg-white">
            <label className="block text-[10px] font-bold mb-1 uppercase">오른쪽 로고 주소</label>
            <input className="w-full border rounded p-2 text-xs font-mono mb-2" value={config.logos.rightLogo} onChange={e => updateLogos('rightLogo', e.target.value)} />
            <label className="block text-[10px] font-bold mb-1 uppercase text-indigo-600">클릭 이동 링크</label>
            <input className="w-full border border-indigo-200 rounded p-2 text-xs font-mono" value={config.logos.rightLink} onChange={e => updateLogos('rightLink', e.target.value)} />
            <SpecGuide spec="600px+ Width, Transparent PNG" />
            <ImagePreview url={config.logos.rightLogo} label="Right Logo" />
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="p-4 border rounded bg-white space-y-4">
          ...
          <input className="w-full border rounded p-2 text-xs font-mono" placeholder="Hero Bg URL" value={config.backgrounds.heroBg} onChange={e => updateBg('heroBg', e.target.value)} />
          ...
          <div className="mt-4">
             ...
             <div className="w-full h-32 rounded border bg-slate-900 overflow-hidden relative">
                {config.backgrounds.heroBg ? (
                   <img 
                    src={getDirectImageUrl(config.backgrounds.heroBg)} 
