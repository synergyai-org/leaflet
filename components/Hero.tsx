...
const Hero: React.FC<HeroProps> = ({ section, accentColor, backgrounds, bgColor }) => {
  ...
  return (
    <section ref={heroRef} className="hero-main relative h-[95vh] flex flex-col justify-center items-center text-center px-4 overflow-hidden">
      <div 
        ref={bgRef}
        className="absolute inset-0 z-0 bg-cover bg-center will-change-transform"
        style={{ 
          backgroundImage: backgrounds.heroBg ? `url('${getDirectImageUrl(backgrounds.heroBg)}')` : 'none',
      ...
      <div 
        ...
        style={{ background: `radial-gradient(circle at center, transparent 0%, ${bgColor}CC 60%, ${bgColor} 100%)` }}
      ></div>
      <div ref={contentRef} className="relative z-20 space-y-12 w-full max-w-[95vw] mx-auto flex flex-col items-center">
        <div className="space-y-4 w-full px-4">
          <RenderStyled data={section.subtitle} isFluid={true} className="opacity-90 block tracking-[3px] uppercase" />
          <h1 className="leading-[1.1] hero-title-gradient drop-shadow-[0_10px_30px_rgba(0,0,0,0.9)] mb-4 w-full">
             <RenderStyled data={section.title} isFluid={true} className="block" />
          </h1>
          <div className="opacity-80 block tracking-[1px] w-full">
             <RenderStyled data={section.mainSubtitle} isFluid={true} className="block italic font-light" />
          </div>
        </div>
