...
const RenderStyled: React.FC<RenderStyledProps> = ({ data, defaultColor, className, fontSize, isFluid }) => {
  ...
  const getOpticalLetterSpacing = () => {
    if (data.letterSpacing && data.letterSpacing !== 'inherit') return data.letterSpacing;
  ...
  return <span style={styles} className={className}>{data.text}</span>;
};
